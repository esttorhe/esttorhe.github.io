...
date: 2015-10-02
category: Tech
description: "Fixing Nokogiri gem installation issues on macOS El Capitan with bundler and libxml2 paths."
tags:
  - nokogiri
  - gems
  - el capitan
...
Finally `El Capitan` arrived and I couldn't be more eager to install it on my machine; as soon as I read on Twitter that it was available I jumped to the `AppStore` and started the download process which seemed like a bad idea at the time:
...
<TweetQuote
  ...
  date="October 2, 2015"
>
  El Capitan download speed: ~7 hours remaining 😅
</TweetQuote>
...
Also if you didn't know `CocoaPods` uses `nokogiri` as a dependency; this means that, if you want to «install» `CocoaPods` you'll end up installing `nokogiri`.
...
It didn't take long for me to find a question on `StackOverflow` that addressed the same issue but for different versions of `nokogiri` and `Xcode` and it was just a matter of putting `2 + 2` together and finally my `bundle install` was successfully installing `CocoaPods` with `nokogiri` as a dependency.
