...
date: 2014-12-16
category: Tech
description: "Investigating a bug with iOS 7 and the new adaptive segues where Show segues present modally instead of pushing."
...
## First Glimpse Of The Problem

Although this "embracing the change" would come to haunt us with a weird bug that was reported to us; hitting a "share" button was yielding no actions…
...
![Segues Table](/assets/images/post/2014/12/show-adaptative-segue-error/segues-table.png)
...
![iOS 8 working correctly](/assets/images/post/2014/12/show-adaptative-segue-error/iOS8.png)

close compile and run on `iOS7` and… there you go.

![iOS 7 broken - modal presentation](/assets/images/post/2014/12/show-adaptative-segue-error/iOS7.png)
...
This is obviously a bug introduced by Apple that does not support "retro compatibility" of this new `adaptive segues`.
...
Recently I got an update on my radar with the following response:
...
So there you have it; kind of accepted it as a bug (at least on IB not warning) but at the same time is telling us that pre `iOS8` adaptive segues could have unexpected behavior.
